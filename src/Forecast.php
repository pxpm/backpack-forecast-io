...
class Forecast
{
    ...
    {
        $weather = Cache::get('forecast_weather', function () {
            if (!is_null(config('settings.forecast_latitude')) && (!is_null(config('settings.forecast_longitude')))) {
                $forecast = $this->get(config('settings.forecast_latitude'), config('settings.forecast_longitude'));
                Cache::put('forecast_weather',$forecast);
                return $forecast;
            }
            return false;
        });
